...
def _build_atempo_chain(factor: float) -> str:
    """
    Builds a chained atempo filter string for FFmpeg.
    FFmpeg's atempo only accepts values in [0.5, 2.0], so factors outside
    that range require multiple chained filters (e.g. 4x → atempo=2.0,atempo=2.0).
    """
    filters = []
    remaining = factor
    while remaining > 2.0:
...
def accelerate_video(
    ...
    Args:
        ...
        progress_callback: Função callback que recebe (progresso: float, mensagem: str)
        cancel_event: threading.Event para cancelamento cooperativo
        preset: Preset de encoding do FFmpeg (padrão: medium)
        crf: Qualidade de encoding 18-28 (padrão: 23)
    ...
    Raises:
        ...
        InterruptedError: Se o processamento for cancelado via cancel_event
    """
    import logging
    ...
    # Detectar FPS da fonte para evitar frame drops em vídeos VFR
    source_fps = _get_video_fps(input_path)
    if source_fps:
        logger.info(f"FPS detectado: {source_fps:.3f}")
    fps_filter = f",fps={source_fps:.3f}" if source_fps else ""

    if mute_audio:
        filter_complex = f"[0:v]setpts={1/fator_aceleracao}*PTS{fps_filter}[v]"
        comando = [
            "ffmpeg",
            "-i", input_path,
        ...
        filter_complex = (
            f"[0:v]setpts={1/fator_aceleracao}*PTS{fps_filter}[v];"
            f"[0:a]{_build_atempo_chain(fator_aceleracao)}[a]"
        )
        comando = [
            "ffmpeg",
            "-i", input_path,
